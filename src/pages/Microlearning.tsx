...
export function Microlearning() {
    ...
    const unsub = onSnapshot(docRef, (docSnap) => {
      if (docSnap.exists()) {
        ...
        setConfig(prev => ({
          ...
          chapters: data.chapters && data.chapters.length > 0 ? data.chapters : prev.chapters
        }));
      }
  ...
  const currentChapter = config.chapters[activeChapterIndex] || config.chapters[0];

  // Simple mandala grid click handler for virtual painting
  const handleColorSegment = (mandalaType: string, segmentIndex: number) => {
    const currentFilled = coloredMandalas[mandalaType] || Array(24).fill('#e2e8f0');
    const updated = [...currentFilled];
    updated[segmentIndex] = selectedColor;
    const newColoredMandalas = { ...coloredMandalas, [mandalaType]: updated };
    ...
    if (!coloredMandalas[mandalaType]) {
  ...
  const renderMandalaSVG = (type: string) => {
    const filledColors = coloredMandalas[type] || Array(24).fill('#f1f5f9');
          ...
          {Array.from({ length: 12 }).map((_, i) => {
            ...
            return (
              <path 
                ...
                stroke="#475569" 
                strokeWidth="1" 
                onClick={() => handleColorSegment(type, i)}
          ...
          {Array.from({ length: 12 }).map((_, i) => {
            ...
            return (
              <circle 
                ...
                stroke="#1e293b" 
                strokeWidth="0.8" 
                onClick={() => handleColorSegment(type, i + 12)}
          ...
          {Array.from({ length: 16 }).map((_, i) => {
            ...
            return (
              <rect
                ...
                stroke="#1e293b"
                strokeWidth="0.8"
                onClick={() => handleColorSegment(type, i + 8)}
                className="hover:opacity-85 transition-all duration-150"
              />
            );
          })}
        ...
        <div className="lg:col-span-8 bg-white border border-slate-200/80 rounded-3xl p-6 md:p-8 shadow-sm">
          ...
          <div className="flex border-b border-slate-100 pb-3 mb-6 gap-2">
            ...
            >
              ...
              {currentChapter.coachingQuestions.some(q => localAnswers[q.id]) && (
            ...
            {activeTab === 'reading' && (
              ...
              >
                <div className="space-y-2">
                  <span className="text-[10px] font-extrabold text-teal-600 bg-teal-50 px-2.5 py-1 rounded-full uppercase tracking-widest">{currentChapter.duration} de lectura activa</span>
                  ...
                  <div key={dyn.id} className="space-y-6">
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-12 gap-6 items-center bg-slate-50 rounded-2xl p-6 border border-slate-100">
                      ...
                      <div className="md:col-span-8 flex justify-center py-4 bg-white rounded-xl border border-slate-100 shadow-inner">
                        {renderMandalaSVG(dyn.mandalaType)}
                      </div>
                            ...
                            <button
                              onClick={() => {
                                if (confirm("¿Seguro que quieres borrar tus trazos en este mándala?")) {
                                  const updated = { ...coloredMandalas, [dyn.mandalaType]: Array(24).fill('#f1f5f9') };
