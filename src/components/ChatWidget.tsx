...
export function ChatWidget() {
  console.log('🔵 [ChatWidget] Componente montado en la aplicación');
  ...
  useEffect(() => {
    console.log('🔵 [ChatWidget] useEffect de eventos registrado');

    // Handler para 'open-kira-chat'
    const handleOpenKiraChat = (e: Event) => {
      console.log('🟢 [ChatWidget] Evento open-kira-chat recibido');
      const customEvent = e as CustomEvent;
      const coach = customEvent.detail?.coach;
      console.log('🟢 Coach recibido:', coach);
      ...
      if (coach) {
        ...
        setIsOpen(true);
        console.log('🟢 Chat abierto con:', resolvedCoach.displayName);
      }
    };

    // Handler para 'open-mentor-chat' (Dashboard)
    const handleOpenMentorChat = (e: Event) => {
      console.log('🔵 [ChatWidget] Evento open-mentor-chat recibido');
      const customEvent = e as CustomEvent;
      const mentor = customEvent.detail;
      console.log('🔵 Mentor recibido:', mentor);
      ...
      if (mentor) {
        ...
        setIsOpen(true);
        console.log('🔵 Chat abierto con mentor:', resolvedMentor.displayName);
      }
    };
    ...
    console.log('✅ [ChatWidget] Listeners registrados');

    return () => {
      console.log('🔴 [ChatWidget] Removiendo listeners');
  ...
  // Resto del código del ChatWidget...
  // (Mantén todas las funciones existentes: fetchContacts, sendMessage, etc.)
  // ...

  const allContacts = [...manualContacts, ...contacts].filter(
  ...
  const handleAcceptOffer = async (msgId: string, offer: any) => {
    ...
    try {
      ...
      if (currentPoints < price) {
        toastError(
          `No tienes suficientes puntos de consciencia (${currentPoints} de ${price} requeridos). ¡Consigue más en tu Diario!`
        );
        return;
      }
            ...
            <div className="flex items-center gap-2">
              ...
              <div>
                ...
                <p className="text-[10px] opacity-80 flex items-center gap-1">
                  ...
                  {selectedContact
                    ? 'Burbuja de Confianza Activa'
                    : `${allContacts.length} mentores activos`}
                </p>
              </div>
            </div>
          ...
          {!selectedContact ? (
            <div className="flex-1 overflow-y-auto p-4 space-y-2">
              <div className="p-3 bg-teal-50/50 border border-teal-100 rounded-2xl mb-3">
                ...
                <p className="text-[11px] text-teal-700 mt-1">
                  Toda comunicación se canaliza de forma privada y segura. Se bloquea cualquier intento de desvío
                  externo para proteger tu privacidad.
                </p>
              </div>
            ...
            <>
              ...
              <div className="flex-1 overflow-y-auto p-4 space-y-4 bg-slate-50/50">
                {messages.length === 0 && (
                  <div className="p-4 bg-teal-50 border border-teal-100 rounded-2xl text-xs space-y-2 text-teal-800 shadow-sm">
                    ...
                    <p>
                      Perfecto. Has solicitado abrir un canal privado con{' '}
                      <strong>{selectedContact.displayName}</strong>.
                    </p>
                    <p>
                      📩 <strong>Chat Privado iniciado</strong> (Solo visible para ti y{' '}
                      {selectedContact.displayName}).
                    </p>
                    <p>
                      🔒 <strong>Recuerda:</strong> Kira Coach actúa como intermediario. No intercambies datos
                      personales hasta que acuerden una sesión formal.
                    </p>
                    <p className="text-[10px] text-teal-600 italic">
                      ✏️ Escribe tu mensaje de presentación aquí abajo (esto es un chat privado, nadie más lo
                      ve):
                    </p>
                  </div>
                )}
                ...
                {messages.map((m) => {
                  ...
                  if (m.type === 'contact_warning') {
                    return (
                      <div
                        key={m.id}
                        className="p-3 bg-red-50 border border-red-100 text-red-800 rounded-2xl text-xs space-y-1 my-2 shadow-sm"
                      >
                  ...
                  if (m.type === 'session_offer') {
                    ...
                    return (
                      <div
                        key={m.id}
                        className="p-4 bg-white border border-slate-200 rounded-2xl space-y-3 shadow-md my-2"
                      >
                        <div className="flex items-center gap-2 border-b border-slate-100 pb-2">
                          <CreditCard className="text-kirateal" size={16} />
                          <span className="font-bold text-xs text-slate-700">
                            Reserva de Sesión Protegida
                          </span>
                        ...
                        <p className="text-xs text-slate-600">
                          <strong>{m.senderName}</strong> ha propuesto una sesión de mentoría privada con las
                        ...
                        <div className="bg-slate-50 p-2.5 rounded-xl text-xs space-y-1.5 border border-slate-100">
                          <p className="flex items-center gap-1.5 text-slate-700">
                            ...
                            <span>
                              <strong>Fecha:</strong> {new Date(offer.date).toLocaleString()}
                            </span>
                          </p>
                          <p className="flex items-center gap-1.5 text-slate-700">
                            <ClockIcon size={13} className="text-slate-400" />
                            <span>
                              <strong>Duración:</strong> {offer.duration} minutos
                            </span>
                          </p>
                          <p className="flex items-center gap-1.5 text-slate-700">
                            ...
                            <span>
                              ...
                              <strong className="text-kirateal">{offer.price} puntos</strong> (protegido por
                              Kira Pay)
                            </span>
                          </p>
                        </div>
                        ...
                        {isAccepted && (
                          <div className="space-y-2 pt-1">
                            ...
                            <div className="flex gap-2">
                              <button
                                onClick={() =>
                                  setShowContactInfo((prev) => ({ ...prev, [m.id]: !prev[m.id] }))
                                }
                            ...
                            {showContactInfo[m.id] && (
                              <div className="p-3 bg-teal-50 rounded-xl border border-teal-100 text-[11px] space-y-1 text-teal-900 animate-in fade-in">
                                <p className="font-bold text-teal-800">
                                  Contacto Seguro verificado por Kira Pay:
                                </p>
                                <p>
                                  <strong>Email:</strong>{' '}
                                  {selectedContact.email || 'contacto@kiracoach.com'}
                                </p>
                                <p>
                                  <strong>Teléfono:</strong>{' '}
                                  {selectedContact.phone || '+54 9 11 5555-0199'}
                                </p>
                                <p className="text-[9px] text-teal-600 italic">
                                  Recuerda mantener el respeto mutuo durante la sesión.
                                </p>
                              </div>
                            )}
                          </div>
                        ...
                        {isCompleted && (
                          <p className="text-[11px] text-kirateal font-bold flex items-center justify-center gap-1 bg-teal-50/50 p-2 rounded-xl">
                            <ShieldCheck size={14} /> Sesión completada con éxito. ¡Intercambio seguro
                            finalizado!
                          </p>
                        )}
                  ...
                  if (m.type === 'session_review') {
                    ...
                    return (
                      <div
                        key={m.id}
                        className="p-4 bg-teal-50/40 border border-teal-100 rounded-2xl space-y-3 shadow-sm my-2"
                      >
                        ...
                        <p className="text-[11px] text-slate-600 leading-relaxed">
                          ¡Sesión completada! Ahora pueden calificarse mutuamente de forma privada para
                          mantener la reputación interna de Kira Coach.
                        </p>
                        ...
                        {isPendingReview ? (
                            ...
                            <div className="space-y-3">
                              ...
                              <div>
                                ...
                                <textarea
                                  value={comments[m.id] || ''}
                                  onChange={(e) =>
                                    setComments((prev) => ({ ...prev, [m.id]: e.target.value }))
                                  }
                          ...
                          <div className="p-2.5 bg-emerald-50 rounded-xl text-[11px] text-emerald-800 space-y-1">
                            ...
                            <p>
                              Tu puntuación de{' '}
                              <strong>{review.rating || ratings[m.id] || 5} estrellas</strong> ha sido
                              resguardada en el registro de confianza.
                            </p>
                          </div>
                        )}
                      </div>
              ...
              <form
                ...
                className="p-4 border-t border-slate-100 flex gap-2 bg-slate-50 rounded-b-3xl"
              >
                <input
                  value={input}
                ...
                <button
                  type="submit"
                  className="p-2.5 bg-kirateal text-white rounded-xl hover:bg-kirateal-dark transition-all active:scale-95 flex items-center justify-center"
                >
                  <Send size={18} />
                </button>
              </form>
